...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && (w.chainConfig.Clique == nil || w.chainConfig.Clique.Period > 0) && (w.chainConfig.CliqueFaster == nil || w.chainConfig.CliqueFaster.PeriodMS > 0) {
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				// CliqueFaster:
				if w.chainConfig.CliqueFaster != nil && w.chainConfig.CliqueFaster.PeriodMS == 0 {
					w.commitWork(nil, time.Now().Unix())
				}
			}
